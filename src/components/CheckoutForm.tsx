...
export default function CheckoutForm({ totalAmount, onClose }: CheckoutFormProps) {
  const [paymentStatus, setPaymentStatus] = useState<'idle' | 'success' | 'error'>('idle');
  const [paymentDescription, setPaymentDescription] = useState<string>('Product Purchase');
  ...
  useEffect(() => {
    if (items.length === 1) {
      ...
      setPaymentDescription(`Purchase: ${mainItem} and ${otherCount} other item${otherCount > 1 ? 's' : ''}`);
    }
  }, [items]);
  ...
  if (paymentStatus === 'success') {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="mt-2 text-xl font-medium text-gray-900 dark:text-white">Payment successful!</h3>
        <p className="mt-1 text-gray-500 dark:text-gray-400">Thank you for your purchase.</p>
        <div className="mt-6">
          <button
            onClick={onClose}
  ...
  return (
    <div className="space-y-6">
      <div className="border-b border-gray-200 dark:border-gray-700 pb-4">
        <h3 className="text-lg font-medium text-gray-900 dark:text-white">Complete Your Purchase</h3>
      ...
      <StandalonePaymentForm
        ...
        amount={totalAmount} /* Using actual amount in dollars instead of cents */
        currency="USD"
